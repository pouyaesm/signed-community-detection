...
public class ListMatrix extends BaseMatrix{
    ...
    public ListMatrix[] decompose(int[] partition){
        ...
        int groupCount = statistics.count;
    ...
    /**
     * Return list of only positive (or negative) values
     * @param isPositive
     * @return
     */
    public ListMatrix getUniSignListMatrix(boolean isPositive){
        int count = 0; // number of valid cells
        for(int p =0 ; p < rows.length ; p++){
            if((isPositive && values[p] > 0) || (!isPositive && values[p] < 0)){
                count++;
            }
        }
        int[] uniRows = new int[count];
        int[] uniColumns = new int[count];
        float[] uniValues = new float[count];
        int insertAt = 0;
        for(int p = 0 ; p < values.length ; p++){
            if((isPositive && values[p] > 0) || (!isPositive && values[p] < 0)){
                uniRows[insertAt] = rows[p];
                uniColumns[insertAt] = columns[p];
                uniValues[insertAt] = values[p];
                insertAt++;
            }
        }
        ListMatrix subList = new ListMatrix().init(uniRows, uniColumns, uniValues, isIdShared())
