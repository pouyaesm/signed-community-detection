...
public class CPM extends RosvallBergstrom {
    ...
    public int[] detect(Graph graph, float resolution, float alpha, int refineCount){
        ...
        return partition(graph, refineCount);
    }
    ...
    protected float greedy(Graph graph, Graph transpose, int[] partition) {
        ...
        // neighborGroup[ng] = q >= 0 means group ng is a neighbor of current group g
        // and it is placed in position q of queue
        int[] neighborGroup = new int[N];
        for(int n = 0 ; n < N ; n++){
            int groupId = partition[n];
            if(neighborGroup[groupId] == 0) {
                groupQueue[groupId] = new float[5];
                neighborGroup[groupId] = -1;
            }
        }
        ...
        while (hamImproved && (movedNodes / N) >= 0.01){
            ...
            for(int k = 0 ; k < N ; k++){
                ...
                // To prevent group from being considered as its own neighbor
                neighborGroup[groupId] = Integer.MAX_VALUE;
                ...
                for(int direction = 0 ; direction < 2 ; direction++){
                    ...
                    for(int n = 0 ; n < neighbors.length ; n++){
                        ...
                        if(neighborGroup[neighborGroupId] == -1){
                            groupQueue[queueHead][0] = neighborGroupId;
                            neighborQueuePosition = neighborGroup[neighborGroupId] = queueHead;
                            queueHead++;
                        }else{
                            neighborQueuePosition = neighborGroup[neighborGroupId];
                        }
                ...
                // node's group is not in the queue just marked to be excluded from neighbor groups
                neighborGroup[groupId] = -1;
                for(int queueIndex = 0 ; queueIndex < queueHead ; queueIndex++){
                    neighborGroup[(int) groupQueue[queueIndex][0]] = -1;
                    for(int statisticsIndex = 0 ; statisticsIndex < 5 ; statisticsIndex++){
                        groupQueue[queueIndex][statisticsIndex] = 0;
                    }
                }
                queueHead = 0; //reset stack header
